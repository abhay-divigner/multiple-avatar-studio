...
document.addEventListener('DOMContentLoaded', function () {
    ...
    async function onSpeechEnd(text) {
        ...
        if (API_CONFIG.RAG_API_URL === "") {
            ...
            try {
                const answer = await getAnswerFromRAG(text, language, sessionData.session_id);
    ...
    async function createSession() {
        ...
        if (sessionToken && sessionToken != '') {
            ...
            opening_text = (opening_text && opening_text != '') ? opening_text : "Hello, how can I help you?";
    ...
    async function toggleInteractionMode() {
        let activeMode = chatBoxContainer.classList.contains('text_mode') ? 'text_mode' : 'voice_mode'
    ...
    async function getAvatarQuestionnaires() {
        let avatar_studio_id = document.getElementById('avatar_studio_id') ? document.getElementById('avatar_studio_id').value : 0;
    ...
    async function getAnswerFromRAG(question = "", language = "en", sessionId = sessionData.session_id) {
        if (!question.trim()) {
    ...
    function addTextToTranscript(text, role = 'avatar', chunk = false, timeStamp = new Date().toISOString()) {
        if (transcriptContainer) {
            ...
            let lastTranscriptElement = transcriptElements.length ? transcriptElements[transcriptElements.length - 1] : null;
            ...
            if (lastTranscriptElement) {
                if (chunk) {
                    ...
                    if (!lastTranscriptElement.textContent || (lastTranscriptElement?.textContent.trim() != text.trim())) {
                        lastTranscriptElement.textContent = `${text}`;
                    }
