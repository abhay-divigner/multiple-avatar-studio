                ...
                <div class="avatar-form-grid-section">
                        ...
                        <div class="form-grid three-columns" style="margin-bottom: 40px;">
                            ...
                            <div class="form-field">
                                ...
                                <input type="number" min="0" name="time_limit" id="time_limit" style="width: 160px;" 
                                    value="<?php echo $avatar && $avatar->time_limit ? esc_attr($avatar->time_limit) : 10  ?>" required />
                    ...
                    <div class="container">
                        ...
                        <div class="form-divider"></div>                
                            ...
                            <script>
                                // Function to check if disclaimer should be required based on user form status
                                function updateDisclaimerRequirements() {
                                    ...
                                    const disclaimerTitle = document.getElementById('disclaimer_title');
                                    const disclaimerCheckbox = document.getElementById('disclaimer_enable');
                                    
                                    if (userFormEnabled) {
                                        // If user form is enabled, disclaimer becomes required
                                        if (!disclaimerEnabled) {
                                            // Force enable disclaimer
                                            disclaimerCheckbox.checked = true;
                                            disclaimerCheckbox.setAttribute('readonly', 'readonly');
                                            disclaimerCheckbox.setAttribute('onclick', 'return false;');
                                            disclaimerCheckbox.style.cursor = 'not-allowed';
                                            
                                            // Show disclaimer fields
                                            toggleDisclaimerFields(true);
                                            
                                            // Add a note
                                            if (!document.getElementById('disclaimer_required_note')) {
                                                const note = document.createElement('div');
                                                note.id = 'disclaimer_required_note';
                                                note.style.cssText = 'color: #da922c; font-size: 12px; font-weight: 600;';
                                                note.textContent = '(Disclaimer is required when User Form is enabled)';
                                                disclaimerCheckbox.parentNode.appendChild(note);
                                            }
                                        }
                                    } else {
                                        // If user form is disabled, restore normal disclaimer behavior
                                        disclaimerCheckbox.removeAttribute('readonly');
                                        disclaimerCheckbox.removeAttribute('onclick');
                                        disclaimerCheckbox.style.cursor = '';
                                        
                                        // Remove the note if it exists
                                        const note = document.getElementById('disclaimer_required_note');
                                        if (note) {
                                            note.remove();
                                        }
                                        
                                        // Reset disclaimer to its saved state
                                        const savedDisclaimerState = <?php echo ($avatar && $avatar->disclaimer_enable) ? 'true' : 'false'; ?>;
                                        disclaimerCheckbox.checked = savedDisclaimerState;
                                        toggleDisclaimerFields(savedDisclaimerState);
                                    }
                                }

                                // Modified toggleDisclaimerFields function
                                ...
                                document.addEventListener('DOMContentLoaded', function() {
                                    ...
                                    // Set initial state
                                    updateDisclaimerRequirements();
                                    
                                    // Monitor user form checkbox changes
                                    if (userFormCheckbox) {
                                    ...
                                    if (form) {
                                        form.addEventListener('submit', function(e) {
                                            ...
                                            // If user form is enabled, disclaimer must be enabled and filled
                                            if (userFormEnabled) {
                                                if (!disclaimerEnabled) {
                                                    alert('Disclaimer is required when User Form is enabled.');
                                                    e.preventDefault();
                                                    return false;
                                                }
                                            }
                                            
                                            // Validate disclaimer fields if disclaimer is enabled
                                            if (disclaimerEnabled) {
                                                // Validate title
                                    ...
                                    <td>
                                        <select name="selected_form_id" id="selected_form_id" class="regular-text" 
                                            ...
                                            foreach ($forms as $form) {
                                                // Display ID and title together
                                                $form_display = sprintf('AS%d - %s', $form->id, esc_html($form->title));
